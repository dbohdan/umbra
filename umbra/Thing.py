...
class Thing:
    ...
    def draw(self, canvas, nx, ny, facing, light):
        if not self.sprite: return  # invisible things
    ...
    def move(self, level0, x0, y0, level1, x1, y1):
        # leave the old grid...
        if level0:
            if not level0.removeStuff(x0, y0, self): return 0
        # move to the new one...
        if level1:
            if not level1.addStuff(x1, y1, self): return 0
...
#            print "%s.setCoord(%d, %d, %d)" % (self.name, levelnum, x, y)
    ...
    def x(self):
        if self.__coord < 0: return self.__coord
