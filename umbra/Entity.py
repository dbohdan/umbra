...
Fisticuffs = Script.Script(
    (Script.T, Script.DMG, Global.Wounds, "1 2"),
)
...
class Entity(Thing.Thing):
    ...
    def __init__(self, name, sprite, species, gender, prof, stat):
        if sprite != None:
            sprite = os.path.join("entity", sprite)
    ...
    def meleeAttackAhead(self, effect):
        ...
        level = game.getLevel()
        d = self.facing
        tx = self.x() + Global.DX[d]
        ty = self.y() + Global.DY[d]
    ...
    def awardXP(self, award):
        ...
        if self.xp >= self.nextXP():
            ...
            self.stat[st] += 1
            self.message(
                "You are now level %d!  Your %s improves, and you have %d skill point%s."
                % (
                    self.level,
    ...
    def charText(self, columns):
        text = "%s %s\nProfession: %s\nLevel: %d\tXP: %d (next level at %d XP)\n\n" % (
            Global.GENDER_NAMES[self.gender],
            self.species,
            Global.PROF_NAMES[self.prof],
            self.level,
            self.xp,
            self.nextXP(),
        )
    ...
    def drawStatus(self, canvas, charfont, tinyfont, y, key):
        ...
        x = 0
        ystats = y
        for st in range(NPRIMES, NSTATS):
            if row == NPRIMES2:
                x = w // 2
                y = ystats
            row += 1
    ...
    def getSkill(self, skill):
        if isinstance(skill, Skill.Skill):
    ...
    def isBurrowing(self):
        return (
            self.levelnum() == 0
            and self.isUndead()
            and Global.umbra.game.light_level >= Global.LIGHT_DAY
        )
    ...
    def readyEquip(self, item):
        ...
        for p in range(Equip.NPOS):
            ...
            if self.player:
                Global.umbra.alert(
                    "Ready Equipment",
                    "Cannot use %s until something else is removed." % item,
                )
            return 0
    ...
    def trigger(self, who):
        ...
        while 1:
            ...
            if x == None:
                ...
                opt = u.menu(title="%s asks:" % self.name, banner=x[0], prompts=x[1])
                self.speechState = x[2][opt]
                if self.speechState < 0:
                    self.speechState = 0
                    return 1
            else:
                assert 0, "Unknown speech type (%s) '%s'" % (type(x), x)
    ...
    def tryAmmo(self, item):
        ...
        if not item.fitsIn(gun):
            self.message("You can't put %s ammo in a %s!" % (item.ammoType, gun.name))
            return 0
    ...
    def witnessAttack(self, victim):
        ...
        if self.player or victim.player:
            notplayers = 0
        else:
            notplayers = 1
        for y in range(y0, y1):
            for x in range(x0, x1):
                ...
                if not wits:
                    continue
                for who in wits:
                    if who is self or who is victim:
                        continue
                    fperp = who.getFriendliness(self)
                    fvictim = who.getFriendliness(victim)
                    if DEBUG_WITNESS:
                        print(
                            "    witness %s, fperp=%s, fvictim=%s, friends=%s"
                    ...
                    if fvictim < Global.F_Neutral:
                        who.friendliness[self.name] = min(Global.F_Friendly, fperp + 1)
                        if DEBUG_WITNESS:
                            print("hooray!")
                    elif fvictim > Global.F_Neutral:
                        who.friendliness[self.name] = max(Global.F_Enmity, fperp - 2)
                        if DEBUG_WITNESS:
                            print("I hate you!")
                    else:
                        if DEBUG_WITNESS:
                    ...
                    if who.player and notplayers:
                        if DEBUG_WITNESS:
                            print("%s sees it!" % who.name)
                        Global.umbra.showGunshot(
                            self.x(),
                            self.y(),
                            victim.x(),
                            victim.y(),
...
class EntityDialog(OkayDialog.OkayDialog):
    def makeBody(self, body, args):
        ...
        # controls
        if main:
            for i in range(1, len(Global.VIEW_MENU)):
