...
class Dungeon:
    ...
    def generate(self):
        ...
        for i in range(self.opts[OPT_ITERATIONS]):
            if DEBUG:
                print(i, end=" ")
            # if there's no blank space available, stop mapping
            if not self.selectWall():
                break
            for j in range(sub_its):
                roll = util.d(1, 100)
                if roll <= closedRoom:
            ...
            # hook.
            if DEBUG:
                print("after %d tries" % j, end=" ")
            if not self.feature:
                self.level.setTerrain(self.xSelect, self.ySelect, Terrain.Dungeon_Wall)
                if DEBUG:
                    print("None")
                continue
            if DEBUG:
    ...
    def createInitialRoom(self):
        ...
        self.feature = DungeonOpenRoom(self, roomx, roomy, util.d(1, Global.NDIRS) - 1)
    ...
    def isBuildableSpace(self, x, y):
        ...
        if ter is None or Terrain.isHook(ter):
            return 1
    ...
    def __stripHook(self, x, y, Floor, Wall):
        ...
        for d in range(Global.NDIRS):
            ...
            if ter == Terrain.Dungeon_Floor or Terrain.isHook(ter):
                count += 1
        if count <= 1:
