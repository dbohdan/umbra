...
class Ruins(MapGenerator.MapGenerator):
    ...
    def makeSkyscraperDoors(self, sector, x0, y0, x1, y1, w, h):
        if DEBUG:
            print("makeSkyscraperDoors %d,%d-%d,%d" % (x0, y0, x1, y1))
        for d in range(Global.NDIRS):
            while 1:
                ...
                dy = y + Global.DY[d]
                if (
                    sector.level[TOWER].getTerrain(dx + Global.DX[d], dy + Global.DY[d])
    ...
    def makeSkyscraperStairs(self, sector, x0, y0, x1, y1, w, h):
        if DEBUG:
            print("makeSkyscraperStairs %d,%d-%d,%d" % (x0, y0, x1, y1))
    ...
    def findStairs(self, lvl0, lvl1, x0, y0, x1, y1):
        ...
        while 1:
            ...
            for d in range(Global.NDIRS):
                if d == facing:
                    goodterrain = (Terrain.Dungeon_Floor,)
                else:
                    goodterrain = okayterrain
                dx = xstairs + Global.DX[d]
                dy = ystairs + Global.DY[d]
    ...
    def makeSkyscraperContents(self, sector, x0, y0, x1, y1, w, h):
        if DEBUG:
            print("makeSkyscraperContents %d,%d-%d,%d" % (x0, y0, x1, y1))
