    ...
    def checkSpace(self):
        for y in range(self.y0 - 1, self.y1 + 1):
            for x in range(self.x0 - 1, self.x1 + 1):
                if not self.dungeon.isBuildableSpace(x, y):
                    return 0
        return 1
...
class DungeonClosedRoom(DungeonAbstractRoom):
    def add(self):
        ...
        self.outline()
        level = self.dungeon.level
        for outdir in range(Global.NDIRS):
    ...
    def add(self):
        ...
        for h in range(nhooks):
            ...
            if self.dungeon.isBuildableSpace(x, y):
                level.setTerrain(x, y, Terrain.hookForDir(outdir))
        level.setTerrain(self.xloc, self.yloc, Terrain.Dungeon_Floor)
        x = self.xloc + DX[self.facing]
        y = self.yloc + DY[self.facing]
        level.setTerrain(x, y, Terrain.Dungeon_Floor)
        x += DX[self.facing]
        y += DY[self.facing]
        level.setTerrain(x, y, Terrain.Dungeon_Floor)
