...
def brainAutopilot(self, turn, level):
    ...
    if (
        foes
        and self.taskStat(Global.Speed) >= 0
        and __tryRanged(self, turn, level, foes)
    ):
        return RES_Ranged
    if victims:
    ...
    if step == "n":
        movedir = Global.North
    elif step == "e":
        movedir = Global.East
    elif step == "s":
        movedir = Global.South
    elif step == "w":
        movedir = Global.West
    elif step == ".":
...
def brainCoward(self, turn, level):
    ...
    if self.taskStat(Global.Speed) >= 0:
        ...
        if okaydirs:
            ...
            nearestdirs.reverse()
            if Global.BRAIN_DEBUG:
                print("    nearest=%s, okay=%s" % (nearestdirs, okaydirs))
...
def __gossip(self, level, friends):
    for who in friends:
        ...
        if not myfriends and not gossip:
            continue
