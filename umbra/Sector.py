...
class Sector:
    ...
    def makeEncounters(self, level, ncritters, table, okayterrain):
        ...
        for y in range(SIZE):
            for x in range(SIZE):
                if (
                    level.getStuff(x, y) == None
                    and level.getTerrain(x, y) in okayterrain
                ):
                    freespace.append((x, y))
        # This should happen rarely, if ever, but infinite loops are a bad
        # thing.
        if ncritters > len(freespace):
            ncritters = len(freespace)
        # Now seed the level with critters
        for icritter in range(ncritters):
    ...
    def makeBorders(self, level, terlist):
        for x in range(1, SIZE - 1):
            level.setTerrain(x, 0, random.choice(terlist))
            level.setTerrain(x, SIZE - 1, random.choice(terlist))
        for y in range(1, SIZE - 1):
            level.setTerrain(0, y, random.choice(terlist))
            level.setTerrain(SIZE - 1, y, random.choice(terlist))
    ...
    def makePaths(self, level, terlist):
        ...
        if sides & 1:
            ...
            y = 0
            while y < SIZE - 1:
                level.setTerrain(x, y, random.choice(terlist))
                if x == 1:
                    dir = util.d(1, 2) - 1
                elif x == SIZE - 2:
                    dir = util.d(1, 2)
    ...
    def makeStairs(
        ...
        if DEBUG:
            print(
                "makeStairs(self, lvl %d, %d, %d, %d, '%s', lvl %d, %d, %d, %d, %s, %s)"
                % (
                    lvl0.levelnum,
                    xstairs0,
                    ystairs0,
